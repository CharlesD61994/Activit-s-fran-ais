...
function ReaderCell({ cell, revealed }: { cell: TreeAnalysisTableCell; revealed: boolean }) {
  if (cell.role === "answer_line") {
    return <span className="worksheet-reader-cell-copy worksheet-reader-answer-line"><span>{cell.text}</span><span className="worksheet-reader-answer-line-rule"/></span>;
  }
  if (cell.role === "checkbox") {
    return <span className="worksheet-reader-cell-copy worksheet-reader-checkbox-cell"><span className="worksheet-reader-checkbox-mark">□</span><span>{cell.text}</span></span>;
  }
...
export function WorksheetReader({ sentence, persistenceKey, finishControl, onCompleteChange }: Props) {
  ...
  const availableSteps = useMemo(() => [...lines.filter((item) => item.interactive !== false).map((item) => `lines:${item.id}`), ...tables.filter(tableHasInteraction).map((table) => `table:${table.id}`)], [lines, tables]);
    ...
    {page && <div className="tree-reader-page-viewport"><div className="tree-reader-page portrait document-template worksheet-reader-page" style={{ aspectRatio: "8.5 / 11", zoom }}>
      ...
      {tables.filter((item) => (item.pageId ?? pages[0]?.id) === pageId).map((table) => {
        ...
        return <div key={table.id} className={`tree-reader-table worksheet-reader-table ${isFixedWorksheetTable(table) ? "fixed-format" : ""} ${interactive && currentStep === stepId ? "active" : ""}`} style={{ left: `${table.x / W * 100}%`, top: `${table.y / H * 100}%`, width: `${width / W * 100}%`, gridTemplateColumns: normalizedColumnWidths(table).map((value) => `${value / width}fr`).join(" "), gridTemplateRows: normalizedRowHeights(table).map((value) => `${value / W * 100}cqw`).join(" ") }}>
          {table.cells.map((cell, index) => { if (cell.columnSpan === 0) return null; const revealed = revealedCells.includes(`${table.id}:${index}`); const inertCell = cell.role === "answer_line" || cell.role === "checkbox"; return <button key={index} type="button" className={`${revealed && cell.isCorrect ? "selected-correct" : ""} role-${cell.role ?? "text"} background-${cell.background ?? "white"}`} style={{ gridColumn: cell.columnSpan && cell.columnSpan > 1 ? `span ${cell.columnSpan}` : undefined, gridRow: cell.rowSpan && cell.rowSpan > 1 ? `span ${cell.rowSpan}` : undefined, color: cell.textColor ?? (cell.background === "black" ? "white" : "black"), fontSize: `${(cell.fontSize ?? 17) / W * 100}cqw`, fontWeight: cell.bold ? 800 : 500, textAlign: cell.textAlign ?? "center", justifyContent: cell.textAlign === "left" ? "flex-start" : cell.textAlign === "right" ? "flex-end" : "center", alignItems: cell.verticalAlign === "top" ? "flex-start" : cell.verticalAlign === "bottom" ? "flex-end" : "center", borderRightWidth: cell.borderWidth ?? 1, borderBottomWidth: cell.borderWidth ?? 1 }} disabled={inertCell || !interactive || (currentStep !== stepId && !done)} onClick={() => toggleCell(table, index)}>{cell.role === "choice" && <span className="worksheet-choice-mark">{revealed && cell.isCorrect ? "☑" : "□"}</span>}<ReaderCell cell={cell} revealed={revealed}/></button>; })}
        </div>;
      })}
      {lines.filter((item) => item.pageId === pageId).map((item) => { const stepId = `lines:${item.id}`; const revealed = completed.includes(stepId); const staticLine = item.interactive === false; const style = { left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${item.width / W * 100}%`, height: `${item.lineCount * item.lineSpacing / H * 100}%` }; const rules = Array.from({ length: item.lineCount }, (_, index) => <span key={index} style={{ top: `${(index + 1) / item.lineCount * 100}%` }}/>); if (staticLine) return <div key={item.id} className="worksheet-answer-lines reader static" style={style}>{rules}</div>; return <button key={item.id} type="button" className={`worksheet-answer-lines reader ${currentStep === stepId ? "active" : ""}`} style={style} disabled={currentStep !== stepId && !revealed} onClick={() => toggleStep(stepId)}>{rules}{revealed && <div className="worksheet-answer-copy" style={{ fontSize: `${item.answerFontSize / W * 100}cqw`, lineHeight: answerLineHeight(item), fontWeight: item.answerBold ? 800 : 400, textAlign: item.answerTextAlign ?? "left" }}>{item.answer}</div>}</button>; })}
      {checkBoxes.filter((item) => item.pageId === pageId).map((item) => <div key={item.id} className={`worksheet-checkbox-mark reader ${item.checked ? "checked" : ""}`} style={{ left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${item.size / W * 100}%`, height: `${item.size / H * 100}%` }}/>)}
    </div></div>}
  </div>;
}
