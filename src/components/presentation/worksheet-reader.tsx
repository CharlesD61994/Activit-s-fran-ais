...
import type { Sentence, TreeAnalysisTable, TreeAnalysisTableCell, TreeAnalysisTextBox } from "@/types";
...
export function WorksheetReader({ sentence, persistenceKey, finishControl, onCompleteChange }: Props) {
  ...
  const availableSteps = useMemo(() => [...lines.map((item) => `lines:${item.id}`), ...tables.filter(tableHasInteraction).map((table) => `table:${table.id}`)], [lines, tables]);
  ...
  useEffect(() => {
    ...
    const index = pages.findIndex((item) => item.id === targetPage);
  ...
  return <div className="worksheet-reader tree-reader">
    ...
    {page && <div className="tree-reader-page-viewport"><div className="tree-reader-page portrait document-template worksheet-reader-page" style={{ aspectRatio: "8.5 / 11", zoom }}>
      ...
      {(page.mainTitle?.enabled ?? true) && <div className="tree-analysis-document-title-banner" style={{ left: `${(page.margins.left - 53) / W * 100}%`, right: `${(page.margins.right - 53) / W * 100}%`, top: `${70 / H * 100}%` }}><div className="tree-analysis-document-title-line">{page.mainTitle?.prefix} <span>–</span> {page.mainTitle?.title}</div><div className="tree-analysis-document-title-label">{page.mainTitle?.subtitle}</div>{page.mainTitle?.scoreTotal !== undefined && <div className="worksheet-main-title-score"><span>Total&nbsp;:</span><span className="worksheet-main-title-score-box"><b>/{page.mainTitle.scoreTotal}</b></span></div>}</div>}
      {page.taskCallout?.enabled && <div className="worksheet-task-callout" style={{ left: `${(page.margins.left - 10) / W * 100}%`, right: `${(page.margins.right - 53) / W * 100}%`, top: `${154 / H * 100}%` }}><Image className="worksheet-task-label-image" src="/worksheet-assets/ta-tache.png" alt="TA TÂCHE" width={80} height={32} unoptimized/><span className="worksheet-task-arrow" aria-hidden/><p>{page.taskCallout.text}</p></div>}
      ...
      {tables.filter((item) => (item.pageId ?? pages[0]?.id) === pageId).map((table) => {
        const stepId = `table:${table.id}`; const done = completed.includes(stepId); const interactive = tableHasInteraction(table); const width = worksheetTableWidth(table);
        return <div key={table.id} className={`tree-reader-table worksheet-reader-table ${isFixedWorksheetTable(table) ? "fixed-format" : ""} ${interactive && currentStep === stepId ? "active" : ""}`} style={{ left: `${table.x / W * 100}%`, top: `${table.y / H * 100}%`, width: `${width / W * 100}%`, gridTemplateColumns: normalizedColumnWidths(table).map((value) => `${value / width}fr`).join(" "), gridTemplateRows: normalizedRowHeights(table).map((value) => `${value / W * 100}cqw`).join(" ") }}>
          {table.cells.map((cell, index) => { if (cell.columnSpan === 0) return null; const revealed = revealedCells.includes(`${table.id}:${index}`); return <button key={index} type="button" className={`${revealed && cell.isCorrect ? "selected-correct" : ""} role-${cell.role ?? "text"} background-${cell.background ?? "white"}`} style={{ gridColumn: cell.columnSpan && cell.columnSpan > 1 ? `span ${cell.columnSpan}` : undefined, gridRow: cell.rowSpan && cell.rowSpan > 1 ? `span ${cell.rowSpan}` : undefined, color: cell.textColor ?? (cell.background === "black" ? "white" : "black"), fontSize: `${(cell.fontSize ?? 17) / W * 100}cqw`, fontWeight: cell.bold ? 800 : 500, textAlign: cell.textAlign ?? "center", justifyContent: cell.textAlign === "left" ? "flex-start" : cell.textAlign === "right" ? "flex-end" : "center", alignItems: cell.verticalAlign === "top" ? "flex-start" : cell.verticalAlign === "bottom" ? "flex-end" : "center", borderRightWidth: cell.borderWidth ?? 1, borderBottomWidth: cell.borderWidth ?? 1 }} disabled={!interactive || (currentStep !== stepId && !done)} onClick={() => toggleCell(table, index)}>{cell.role === "choice" && <span className="worksheet-choice-mark">{revealed && cell.isCorrect ? "☑" : "□"}</span>}<ReaderCell cell={cell} revealed={revealed}/></button>; })}
        </div>;
      })}
      {lines.filter((item) => item.pageId === pageId).map((item) => { const stepId = `lines:${item.id}`; const revealed = completed.includes(stepId); return <button key={item.id} type="button" className={`worksheet-answer-lines reader ${currentStep === stepId ? "active" : ""}`} style={{ left: `${item.x / W * 100}%`, top: `${item.y / H * 100}%`, width: `${item.width / W * 100}%`, height: `${item.lineCount * item.lineSpacing / H * 100}%` }} disabled={currentStep !== stepId && !revealed} onClick={() => toggleStep(stepId)}>{Array.from({ length: item.lineCount }, (_, index) => <span key={index} style={{ top: `${(index + 1) / item.lineCount * 100}%` }}/>) }{revealed && <div className="worksheet-answer-copy" style={{ fontSize: `${item.answerFontSize / W * 100}cqw`, lineHeight: item.lineSpacing / item.answerFontSize, fontWeight: item.answerBold ? 800 : 400, textAlign: item.answerTextAlign ?? "left" }}>{item.answer}</div>}</button>; })}
    </div></div>}
  </div>;
}
