    ...
    <div className="tree-reader">
      ...
      <div className={`tree-reader-page ${currentInteraction ? "drawing" : ""}`} onClick={handleDrawingClick} onMouseMove={(event) => { if (!drawingStart) return; const rect = event.currentTarget.getBoundingClientRect(); setDrawingCurrent({ x: event.clientX - rect.left, y: event.clientY - rect.top }); }}>
        {textBoxes.map((box) => {
          ...
          return <div key={box.id} className="tree-reader-text" style={{ left: `${box.x / 1056 * 100}%`, top: `${(box.y - topOffset) / 816 * 100}%`, width: `${box.width / 1056 * 100}%`, fontSize: `${box.fontSize / 1056 * 100}cqw` }}>{boundaries.slice(0, -1).map((segmentStart, segmentIndex) => { const segmentEnd = boundaries[segmentIndex + 1]; const framed = answerRanges.some((item) => item.start <= segmentStart && item.end >= segmentEnd); let tokenOffset = segmentStart; const tokens = box.text.slice(segmentStart, segmentEnd).match(/\S+|\s+/g) ?? []; return <span key={`${segmentStart}-${segmentEnd}`} className={framed ? "tree-reader-framed" : undefined}>{tokens.map((token, tokenIndex) => { const start = tokenOffset; const end = start + token.length; tokenOffset = end; return /^\s+$/u.test(token) ? token : <span key={`${tokenIndex}-${start}`} className="tree-reader-word" data-box-id={box.id} data-start={start} data-end={end}>{token}</span>; })}</span>; })}</div>;
        })}
        ...
        {tables.map((table) => { const tableDone = completed.includes(`table:${table.id}`); return <div key={table.id} className={`tree-reader-table ${currentTable?.id === table.id ? "active" : ""}`} style={{ left: `${table.x / 1056 * 100}%`, top: `${(table.y - topOffset) / 816 * 100}%`, gridTemplateColumns: `repeat(${table.columns},1fr)` }}>{table.cells.map((cell, index) => cell.columnSpan === 0 ? null : <button type="button" key={index} className={tableDone && cell.isCorrect ? "selected-correct" : ""} style={{ gridColumn: cell.columnSpan && cell.columnSpan > 1 ? `span ${cell.columnSpan}` : undefined }} disabled={currentTable?.id !== table.id} onClick={(event) => { event.stopPropagation(); if (cell.isCorrect) completeStep(`table:${table.id}`); else setFeedback("Ce n’est pas la bonne cellule."); }}>{cell.text}</button>)}</div>; })}
